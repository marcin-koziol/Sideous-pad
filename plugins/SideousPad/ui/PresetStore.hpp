/*
 * Sideous Pad - simple file-based preset storage, entirely UI-side (no DPF
 ...
 * array). One plain-text ".sppreset" file per preset, one "symbol=value"
 ...
 * same failure across every sideous-* plugin that copied this file, see
 * sideous-noise's fix). POSIX mkdir/opendir/readdir/remove carry no such
 * version gating.
 */
...
inline std::string presetsDirectory()
{
    ...
    return appData ? std::string(appData) + "\\sideous-pad\\presets" : ".\\presets";
#elif defined(__APPLE__)
    const char* home = std::getenv("HOME");
    return home ? std::string(home) + "/Library/Application Support/sideous-pad/presets" : "./presets";
#else
    if (const char* xdg = std::getenv("XDG_DATA_HOME"))
        return std::string(xdg) + "/sideous-pad/presets";
    const char* home = std::getenv("HOME");
    return home ? std::string(home) + "/.local/share/sideous-pad/presets" : "./presets";
#endif
}
...
// sorted (alphabetical) list of preset display names - the ".sppreset"
...
inline std::vector<std::string> listPresets()
{
    ...
    static constexpr const char* kExt = ".sppreset";
...
inline bool savePreset(const std::string& name, const float* values)
{
    ...
    std::ofstream out(presetsDirectory() + "/" + name + ".sppreset");
    if (!out.is_open())
        return false;
...
{
    std::ifstream in(presetsDirectory() + "/" + name + ".sppreset");
    if (!in.is_open())
        return false;
...
{
    return std::remove((presetsDirectory() + "/" + name + ".sppreset").c_str()) == 0;
}
